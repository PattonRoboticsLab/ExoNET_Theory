function legendObjects = PlotPin(Exo, Actual_Pin, shoulder_pin, shoulder_el, shoulder_sw, elbow_sw, elbow_el, bestP_3D, center_back, wrist, Pos)
    
    %% Evaluates numbers of parameters and params per joint
    nparams = Exo.nParamsSh * round(Exo.numbconstraints(2)); 

    p = zeros(nparams+1, 2);          p_wr = bestP_3D(end-2:end);
    p(:,1) = bestP_3D( 1:nparams+1 ); p(:,2) = bestP_3D( nparams+2:nparams*2+2 );
    legendObjects = []; 
    
    %% Change pin 
    Actual_Pin.elevation = Actual_Pin.elevation - Pos.sh + shoulder_pin;
       Actual_Pin.swivel = Actual_Pin.swivel - Pos.sh +  shoulder_pin;

    %% Plot the pin for elevation
    for i = 1:round( p(1,1) )
    pinelevation = plot3( [ Actual_Pin.elevation(i,1), Actual_Pin.elevation(i,1) ], ... % on same axis
                          [ shoulder_pin(2),           Actual_Pin.elevation(i,2) ], ...
                          [ shoulder_pin(3),           Actual_Pin.elevation(i,3) ], ...
                          'm',  'linewidth', 3, 'DisplayName', 'upperarm_{int}' ); 

    plot3( [ elbow_el(1), Actual_Pin.elevation(i,1) ], ...
           [ elbow_el(2), Actual_Pin.elevation(i,2) ], ...
           [ elbow_el(3), Actual_Pin.elevation(i,3) ], ...
           'm', 'linewidth', 1 ); 
   
    scatter3( elbow_el(1), elbow_el(2), elbow_el(3) , 'mo' );
    
    end
    if exist('pinelevation', 'var') && ~isempty(pinelevation)
    ...
    plot3( [ elbow_sw(1) ,  Actual_Pin.swivel(i,1) ], ...
           [ elbow_sw(2) ,  Actual_Pin.swivel(i,2) ], ...
           [ elbow_sw(3) ,  Actual_Pin.swivel(i,3) ], 'k', 'linewidth', 1 ); 

    scatter3(elbow_sw(1) , elbow_sw(2), elbow_sw(3), 'ko' );   
        
    end    
    if exist('pinswivel', 'var') && ~isempty(pinswivel)
        legendObjects(end+1) = pinswivel;
    end    

    %% Plot the pin for wrist
    pinwrist = plot3( [ elbow_sw(1) ,  wrist(1)   ], ...
                      [ elbow_sw(2) ,  wrist(2)   ], ...
                      [ elbow_sw(3) ,  wrist(3)   ], ...
    ...
    scatter3( elbow_sw(1) + ( shoulder_pin(1)-elbow_sw(1) ) * ( 1-p_wr(2) ), ...
              elbow_sw(2) + ( shoulder_pin(2)-elbow_sw(2) ) * ( 1-p_wr(2) ), ...
              elbow_sw(3) + ( shoulder_pin(3)-elbow_sw(3) ) * ( 1-p_wr(2) ), 'co' );
