...
for test_point=1:size(PHIs,1) %fprintf('\n point %d..',i); % loop for each position
  ...
  for element=1:Exo.nElements,  %fprintf(' element %d..',element);
    ...
    theta=p(shIndex+(element-1)*Exo.nParams+1) + PHIs(test_point,1);
  ...
  for element=1:Exo.nElements,  %fprintf(' element %d..',element);
    ...
    theta= p(elIndex+(element-1)*Exo.nParams+1)+PHIs(test_point,2);
  ...
  if Exo.nJnts==3, taus=[0 0]; %fprintf(' 2 joint..');     
     for element=1:Exo.nElements,  %fprintf(' element %d..',element);
      ...
      theta=p(shElIndex+(element-1)*Exo.nParams+1) + PHIs(test_point,1);
