...
function [Actual_Pin, bestP_3D, bestCost, TauExo, L_springs, L0, Tension, Force_vector, RMSE_exo ] = robustOpto3D_opt( Bod, Pos, Exo, PHIs, TAUsDesired, robot, q )
    ...
    %% Evaluate Force vectors desired
    Force_desired_flexed = plotVectField3D_opt(q, Bod, Pos, Exo, robot, TAUsDesired.TauSh_tot, TAUsDesired.TauEl_tot, ...
                    [1, 0, 0], 1.5,   4, 2, 'IsDesired', true, 'Transparency', 0.9, 'Offset', [0.001, 0.001, 0.001], 'NoPlot', true);  
    Force_desired_extended = plotVectField3D_opt(q, Bod, Pos, Exo, robot, TAUsDesired.TauSh_tot,  TAUsDesired.TauEl_tot, ...
                                    [1, 0, 0], 1.5,   4, 1, 'IsDesired', true, 'Transparency', 0.9, 'Offset', [0.001, 0.001, 0.001], 'NoPlot', true); 
    
    %% Define cost function with error handling 
    cost_function = @(x) cost(x, TAUsDesired, Exo, Pos, Bod, robot, q, Force_desired_flexed, Force_desired_extended);
    ...
    [p_final, bestCost, exitflag_fmincon, output_fmincon] = fmincon(cost_function, p_ga, [], [], [], [], ...
                                                                    lb, ub, [], opts_fmincon);
    ...
    [TauExo, Actual_Pin, L_springs, L0, Tension, func_tension ] = exoNetTorques3D( Pos, Bod, Exo, bestP_3D); % Recall the functions to update the final torques with best cost
    
    %% Plot the values of the best solution
    fprintf(' \nParameters best solution: \n')
    fprintf(' c = %g ', bestCost)
    fprintf('\n                   R    Theta     X      L0 ') %    K    K_rec  L0_rec L_pre_rec')
    ...
    fprintf('\n                   L0_wr  Perc_wr  ')
    fprintf('\n P_elbow       = '); fprintf('%.3f   ', bestP_3D( end-1:end)' ); 
    fprintf('\n');
    ...
    Force_vector.exo_extended = plotVectField3D_opt(q, Bod, Pos, Exo, robot, TauExo.elevationSh, TauExo.elevationEl, ...
                                    [0, 0, 1], 1.5,   4, 1, 'Transparency', 1);
    Force_vector.desired_extended = plotVectField3D_opt(q, Bod, Pos, Exo, robot, TAUsDesired.TauSh_tot,  TAUsDesired.TauEl_tot, ...
                                    [1, 0, 0], 1.5,   4, 1, 'IsDesired', true, 'Transparency', 1, 'Offset', [0.001, 0.001, 0.001]); zlim([0.6 1.9]); hold off; % grid off; legend(legend4);
    ...
    Force_vector.exo_flexed = plotVectField3D_opt(q, Bod, Pos, Exo, robot, TauExo.elevationSh, TauExo.elevationEl, ...
                    [0, 0, 1], 1.5,   4, 2, 'Transparency', 1);
    Force_vector.desired_flexed = plotVectField3D_opt(q, Bod, Pos, Exo, robot, TAUsDesired.TauSh_tot, TAUsDesired.TauEl_tot, ...
                    [1, 0, 0], 1.5,   4, 2, 'IsDesired', true, 'Transparency', 1, 'Offset', [0.001, 0.001, 0.001]); zlim([0.6 1.9]); hold off; % grid off; legend(legend5); 

    %% Figure only pins
    figure('Name','Show exo'); hold on;
    legend6 = PlotPin( Exo, Actual_Pin, shoulder_pin, shoulder_el, shoulder_sw, elbow_sw, elbow_el, bestP_3D, center_back, wrist,Pos);
    ...
    scatter(timetorque, Tau_exo_sh_firstplane,     'b'); 
    scatter(timetorque, Tau_desired_sh_firstplane, 'r');
    ...
    scatter(timetorque, Tau_exo_sh_secondplane,     'b'); 
    scatter(timetorque, Tau_desired_sh_secondplane, 'r');
