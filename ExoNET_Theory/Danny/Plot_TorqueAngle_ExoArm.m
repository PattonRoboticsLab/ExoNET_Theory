% Program to plot Torque generated by single element ExoNET versus angle of
% point of attachment
% Average human mass ~ 62 kg
% Arm length ~ 12 in = 0.3048 m

%% Clear Workspace, Command Window, etc.
clc 
clear all
close all
%% Input Variables
L                  = input('Length of arm = ') ;                                                  % Length of arm [ m ]          
m                  = input('Mass of patient = ') ;                                                % Mass of patient [ kg ]
R                  = input('Element attachment point distance from elbow joint = ') ;             % Element attachment point distance from elbow joint [ m ] 
theta              = input('Angle of elastic element attachment point relative to forearm = ') ;  % Angle of elastic element attachment point relative to humerus [ deg ]
k                  = input('Spring Coefficient = ') ;                                             % Spring Coefficient of elastic element [ N / m ]
x0                 = input('Resting length of elastic element = ') ;                              % Resting Length of elastic element [ m ]
x                  = input('Final length of elastic element = ') ;                                % Final length of elastic element [ m ]

%% Call Function to Calculate Torque
[Phi_Exo, Tau_Exo] = ExoNET1_TorqueAngle(L,R,theta,k,x0,x) ;                                      % Use function to calculate torque and extract variables for ExoNET
[Phi_Arm, Tau_Arm] = Arm1_TorqueAngle(L,m) ;                                                      % Use function to calculate torque and extract variables for forearm
%% Plot Torque vs Input Angle of Forearm Position
figure                                                                                            % Assign space for plot
plot(Phi_Exo,Tau_Exo,'.k',Phi_Arm,Tau_Arm,'.r') ;                                                 % Function to plot input variables
title('Generated Torque vs Angular Position of Forearm') ;                                        % Assign title to generated plot
legend({'ExoNET','Forearm'}) ;                                                                    % Legend to label different lines in plot
xlabel('Phi [Degrees]') ;                                                                         % Assign x-axis label to generated plot
ylabel('Tau [Nm]') ;                                                                              % Assign y-axis label to generated plot
