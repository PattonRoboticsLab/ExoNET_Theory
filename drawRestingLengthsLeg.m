% **************************************************************************
% Draw the resting lengths of MARIONETs for the leg pose specified by phis
% **************************************************************************

function h = drawRestingLengthsLeg(p,phis)
...
for element = 1:EXONET.nElements
    ...
    knee = [BODY.Lengths(1)*sind(phis(1)), ...     % KNEE position
            -(BODY.Lengths(1)*cosd(phis(1)))];
    L0dir = [rPos(1) rPos(2)] - [knee(1) knee(2)]; % L0 direction
    L0dist = norm(L0dir);
    L0dir = L0dir./L0dist;
    plot([knee(1) knee(1)+(L0.*L0dir(1))],[knee(2) knee(2)+(L0.*L0dir(2))],'Color',Colors(1,:),'Linewidth',LW);
...
for element = 1:EXONET.nElements
    ...
    ankle = [knee(1) + BODY.Lengths(2)*sind(phis(1)-phis(2)), ... % ANKLE position
             knee(2) - BODY.Lengths(2)*cosd(phis(1)-phis(2))];
    L0dir = [rPos(1) rPos(2)] - [ankle(1) ankle(2)]; % L0 direction
    L0dist = norm(L0dir);
    L0dir = L0dir./L0dist;
    plot([ankle(1) ankle(1)+(L0.*L0dir(1))],[ankle(2) ankle(2)+(L0.*L0dir(2))],'Color',Colors(2,:),'Linewidth',LW);
...
if EXONET.nJoints == 3
    for element = 1:EXONET.nElements
        ...
        L0dist = norm(L0dir);
        L0dir = L0dir./L0dist;
        plot([ankle(1) ankle(1)+(L0.*L0dir(1))],[ankle(2) ankle(2)+(L0.*L0dir(2))],'Color',Colors(3,:),'Linewidth',LW);
