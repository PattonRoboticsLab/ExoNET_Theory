...
% SYNTAX:	pic=make_xygraph2(filename, draw) 
% DIRECTIONS: 
...
% INPUTS:	filename 	input bmp file. Must have extention  
...
%____________ LOAD FILE ___________
if(nargin>0)
  fprintf(' loading %s, Wait...',filename); pause(.01); 
   [BMP,map]=imread(filename);
   imshow(BMP,map); hold on
  % [BMP,map]=loadbmp(filename); colormap(map); BMP=flipud(BMP);
  % clf; plot(0,0); hold; image(BMP); axis('equal'); 
  ax=axis; 
