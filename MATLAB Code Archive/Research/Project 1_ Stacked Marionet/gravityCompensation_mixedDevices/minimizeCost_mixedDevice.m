function  [optimal_param, optimalTorque_sh, optimalTorque_el, averageErr_el, averageErr_sh, averagePerc_el, averagePerc_sh, Rsquared_el, Rsquared_sh]...
    = minimizeCost_mixedDevice(p, options, mean_err_old)
    %04/16/2018
    global desiredTorque_el desiredTorque_sh phi_1 phi_2 n_sh
    max_iter = 10;
    it = 1;
    toll = 1e-6;
    p_new = fminsearch('torqueCost_mixedDevice', p, options);
    [torque_sh, torque_el] = stacks_mixedDevice(p_new, phi_1, phi_2);
    ...
    while (((err_diff > toll) || (mean_err_new > mean_err_old)) && (it < max_iter)) %when the difference between the new and the old costs decreases under a cetain threshold,
        ...
        p_old = p_new;
        p_new = fminsearch('torqueCost_mixedDevice', p_old, options);
    ...
    optimal_param = p_new;
    [optimalTorque_sh, optimalTorque_el] = stacks_mixedDevice(optimal_param, phi_1, phi_2);
    for i = 1:n_sh
