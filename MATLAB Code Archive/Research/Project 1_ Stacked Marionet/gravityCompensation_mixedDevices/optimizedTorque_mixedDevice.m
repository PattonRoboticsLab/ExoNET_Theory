function [bestParam, averageErr_el, averageErr_sh, averagePerc_el, averagePerc_sh, Rsquared_el, Rsquared_sh] =...
    optimizedTorque_mixedDevice(Lengths, Weights, shoulder_angle, elbow_angle)
    
    %% Declaring variables
    global phi_1 phi_2 desiredTorque_el desiredTorque_sh nTwo_stackedMarionet momentArm_sh...
        momentArm_el spring_length L_ForeArm L_UpperArm n_sh n_el momentArm_singleEl springLength_singleEl...
        momentArm_singleSh springLength_singleSh nSh_stackedMarionet nEl_stackedMarionet
    ...
    init_param = rand(1,(nTwo_stackedMarionet*2+nSh_stackedMarionet*2+nEl_stackedMarionet*2));
    ...
    for i = 1:n_sh
        ...
        desiredMatrix_sh(i, :) = (upperArm_weight/2+foreArm_weight+hand_weight)*...
            L_UpperArm*cos(shoulder_angle)+desiredMatrix_el(i, :);
    ...
    momentArm_singleEl = @(p, phi) (L_ForeArm*p(2)*sin(p(1)-phi))./sqrt(p(2)^2+L_ForeArm^2-(2*p(2)*L_ForeArm*cos(p(1)-phi))); %arm for the force 
    springLength_singleEl = @(p, phi) sqrt(L_ForeArm^2+p(2)^2-2*L_ForeArm*p(2).*cos(p(1)-phi)); %spring length
    
    momentArm_singleSh = @(p, phi) (L_UpperArm*p(2)*sin(p(1)-phi))./sqrt(p(2)^2+L_UpperArm^2-(2*p(2)*L_UpperArm*cos(p(1)-phi))); %arm for the force 
    springLength_singleSh = @(p, phi) sqrt(L_UpperArm^2+p(2)^2-2*L_UpperArm*p(2).*cos(p(1)-phi)); %spring length
    
    [startingTorque_sh, startingTorque_el] = stacks_mixedDevice(init_param, phi_1, phi_2); %torque with random guess (matrix!)
    ...
    [bestParam, optimalTorque_sh, optimalTorque_el, averageErr_el, averageErr_sh, averagePerc_el, averagePerc_sh,...
        Rsquared_el, Rsquared_sh] = minimizeCost_mixedDevice(init_param, options, meanErr_tot);
    
    %% Vector field plot construction
    ct_i = 0;
