function optimizedTorque_gravityField(Lengths, Weights, shoulder_angle, elbow_angle)
    ...
    forceMatrix_general = zeros(2*(length(shoulder_angle)-1), length(elbow_angle)*2);
    ct_i = 0; 
    ...
    for i = 1:length(shoulder_angle)-1
        ...
        optimalParam_sh = minimize_Cost_new(init_param, options, mean_err_old_sh);
        ...
        for j = 1:length(elbow_angle)
            ...
            torqueVector_optim = [optimTorque_sh(j); optimTorque_el(j)];
            ...
            %% Desired forces
            torqueVector_desired = [desiredTau_sh(j); desiredTau_el(j)];
