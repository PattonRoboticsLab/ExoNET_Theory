...
%
%     modified by patton to make it compatible with release 12 (v.6) and to
...
while isempty(data)|(nxtindex==1)
  ...
  line = fgetl(fid);
  if ~isstr(line)
    fprintf(' (%s contains header but no data) ',file)
    break
  end;
  [data, ncols, errmsg, nxtindex] = sscanf(line, '%f');
end % while
header(1,:)=[]; % this is because for initialization there is always an extra line inserted
