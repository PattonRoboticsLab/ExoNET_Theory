function [p_new] = minimizeCost_updateFigures4param(p,options,joint,animations)

%% function objective
% this function minimizes the cost given by the 'torque_cost.m' function
% and starts the animations
%the minimization process stops when the difference between average error
%at the current and the on at the previous iteration is smaller than a
%certain tolerance

    global phi h torque desired_torque err g fig1 fig2 n mean_err_old L_2 L 
    ...
    %First iteration
    p_new = fminsearch('torqueCost_4param', p, options);
    torque = stacks_4param(p_new, phi);
    ...
    if (animations)
        ...
        for i = 1:n
            f = draw_marionet_handles(radius(i), angle(i), stiffness(i), starting_point, ending_point);
            handles(i, :) = f;
        end
    ...
    while (((err_diff > toll) || (mean_err_new > mean_err_old)) && (it < max_iter)) %when the difference between the new and the old average error 
        ...
        p_old = p_new;
        p_new = fminsearch('torqueCost_4param', p_old, options);
        torque = stacks_4param(p_new, phi);
        err = abs(desired_torque - torque);
        mean_err_new = mean(err);
        err_diff = abs(mean_err_new-mean_err_old);
        ...
        if (animations)
            ...
            %updating each marionet
 
            for i=1:n
                draw_marionet_update(radius(i), angle(i), stiffness(i), starting_point, ending_point, handles(i, :));
            end
            drawnow
        end
    end
